...
const PatientList = () => {
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Paper elevation={3} sx={{ p: 2, mb: 3, position: "relative", display: "flex", alignItems: "center" }}>
        <Typography variant="h4" fontWeight="bold" sx={{ position: "absolute", left: "50%", transform: "translateX(-50%)" }}>
          Patients Details
        </Typography>
            ...
            <Grid item xs={12} sm={6} md={4} key={p.id}>
              <Card sx={{ height: "100%", transition: "0.3s", "&:hover": { transform: "scale(1.03)", boxShadow: 6 } }}>
                <CardContent>
                  ...
                  <Button fullWidth variant="contained" sx={{ mt: 2 }} onClick={() => navigate(`/admin/patients/${p.id}`)}>
                    View Details
                  </Button>
                </CardContent>
              </Card>
